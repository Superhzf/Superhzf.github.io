...
The problem is that the indicator function is not continuous or convex making the problem is no longer a quadratic optimization problem.
...
$$\underset{\mathbf{w},b}{min}\quad\frac{1}{m}\sum_{i=1}^mmax(0,1-y_i(\mathbf{w^\top x_i}+b))+\frac{\lambda}{2}||\mathbf{w}||^2$$

Proof: Based on (25) in **theorem 10**, let $\xi_i=max(0, 1-y_i(\mathbf{w^\top x_i}+b))$ and $\lambda = \frac{m}{C}$.

**Definition 7.** The definition of Hinge Loss is:

$$l(s)=max(0, 1-s)$$
where $s = y_i\hat{y_i}=y_i(\mathbf{w^\top x_i}+b)$
