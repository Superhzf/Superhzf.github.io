...
* The outer loop selects the first $\alpha_i$, the inner loop selects the second $\alpha_i$ that maximizes $|E_1-E_2|$
...
* Given the first $\alpha_i$, the inner loop looks for a non-boundary sample that maximizes $|E_2-E_1|$. If this does not make progress, it starts a sequential scan through all the samples.

Based on my experience, it still works if I choose the second $\alpha_i$ which has the maximum prediction error instead of the max $|E_1-E_2|$.
